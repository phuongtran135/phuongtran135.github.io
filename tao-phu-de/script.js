document.addEventListener('DOMContentLoaded', () => {
    ...
    const threadStatusContainer = document.getElementById('thread-status-container');
    ...
    const secondaryGenerationOptions = document.getElementById('secondary-generation-options');
    ...
    function renderApiKeys() {
        ...
        if (apiKeys.length === 0) {
            savedKeysList.innerHTML = '<li class="text-gray-500">Chưa có key nào được lưu.</li>';
            return;
        }
        apiKeys.forEach((key, index) => {
            const li = document.createElement('li');
            li.className = 'flex justify-between items-center p-1 hover:bg-gray-700/50 rounded';
    ...
    async function runParallelVietnameseGeneration() {
        ...
        const processChunk = async (chunk, index) => {
            const chunkSrt = chunk.map((block, i) => `${i + 1}\n${block.time}\n${block.text}`).join('\n\n');
            const prompt = `Bạn là một chuyên gia so khớp phụ đề. Dựa vào nội dung truyện đầy đủ và một đoạn trích từ file phụ đề SRT mẫu, hãy chỉnh sửa lại phần nội dung text trong đoạn trích SRT sao cho khớp chính xác với nội dung truyện. **TUYỆT ĐỐI GIỮ NGUYÊN MỐC THỜI GIAN**.
**Nội dung truyện đầy đủ (để tham khảo ngữ cảnh):**
---
${escapeForPrompt(mainText)}
---
**Đoạn trích SRT cần chỉnh sửa:**
---
${escapeForPrompt(chunkSrt)}
---
**Yêu cầu:** Trả về kết quả dưới dạng JSON object với một key duy nhất là "srt" chứa nội dung SRT của đoạn trích đã được chỉnh sửa.`;
            ...
            if (!result.success) {
                ...
                if (backupKey) {
                    ...
                    setLoadingState(true, `Luồng ${index + 1} đang thử lại với key dự phòng...`);
                    result = await callGeminiApi(prompt, backupKey);
                }
            }
            
            if (result.success) updateThreadStatus(index, 'success');
            else updateThreadStatus(index, 'error', result.error);
        ...
        const workerPromises = chunks.map((chunk, index) => {
            return new Promise(resolve => {
                setTimeout(() => {
                    setLoadingState(true, `Khởi chạy luồng ${index + 1}/${chunks.length}...`);
        ...
        if (failedChunks.length > 0) {
            setLoadingState(true, `Phát hiện ${failedChunks.length} luồng lỗi, bắt đầu quá trình sửa lỗi cuối cùng...`);
            const finalRetryPromises = failedChunks.map(async (failed) => {
                let finalResult = failed;
                for (const key of apiKeys) {
                    updateThreadStatus(failed.originalIndex, 'retrying', 'Vòng cuối');
                    setLoadingState(true, `Thử lại luồng ${failed.originalIndex + 1} lần cuối...`);
                    const chunkSrt = chunks[failed.originalIndex].map((block, i) => `${i + 1}\n${block.time}\n${block.text}`).join('\n\n');
                    const prompt = `Bạn là một chuyên gia so khớp phụ đề. Dựa vào nội dung truyện đầy đủ và một đoạn trích từ file phụ đề SRT mẫu, hãy chỉnh sửa lại phần nội dung text trong đoạn trích SRT sao cho khớp chính xác với nội dung truyện. **TUYỆT ĐỐI GIỮ NGUYÊN MỐC THỜI GIAN**.
**Nội dung truyện đầy đủ (để tham khảo ngữ cảnh):**
---
${escapeForPrompt(mainText)}
...
**Yêu cầu:** Trả về kết quả dưới dạng JSON object với một key duy nhất là "srt" chứa nội dung SRT của đoạn trích đã được chỉnh sửa.`;
                    ...
                    if (result.success) {
                        finalResult = { ...result, originalIndex: failed.originalIndex };
            ...
            const retriedResults = await Promise.all(finalRetryPromises);
            results = results.map(r => retriedResults.find(rr => rr.originalIndex === r.originalIndex && rr.success) || r);
        }
        
        failedChunks = results.filter(r => !r.success);
        if (failedChunks.length > 0) {
            showToast(`Không thể hoàn thành ${failedChunks.length} phần của phụ đề.`, 'error');
            setLoadingState(false);
            return;
        }

        results.sort((a, b) => a.originalIndex - b.originalIndex);
        const allBlocks = results.flatMap(r => parseSrt(r.data.srt));
        const finalSrt = allBlocks.map((block, index) => `${index + 1}\n${block.time}\n${block.text}`).join('\n\n');
    ...
    async function runSingleThreadGeneration(lang) {
        ...
        if (!isInitialGeneration && (!sourceSrt || !mainText)) {
            showToast('Cần có phụ đề tiếng Việt và nội dung truyện gốc để dịch.', 'error');
            return;
        }
        ...
        if (isInitialGeneration) {
            ...
            button = document.querySelector(`.secondary-generate-btn[data-lang="${lang}"], .regenerate-btn[data-lang="${lang}"]`);
        ...
        const languageForPrompt = isInitialGeneration ? 'vietnamese' : langMap[lang];
        
        const prompt = buildSingleThreadPrompt(languageForPrompt, mainText, sourceSrt);
        if (!prompt) { setLoadingState(false); return; }
        ...
        if (result.success) {
            ...
            if (finalSrt) {
                ...
                if (isInitialGeneration) secondaryGenerationOptions.classList.remove('hidden');
                if (status) status.textContent = 'Hoàn thành.';
            } else {
                 showToast(`Lỗi: AI trả về dữ liệu không hợp lệ cho ${lang.toUpperCase()}.`, 'error');
                 if(status) status.textContent = 'Lỗi dữ liệu.';
            }
        } else {
            showToast(result.error || `Tạo phụ đề ${lang.toUpperCase()} thất bại.`, 'error');
    ...
    function buildSingleThreadPrompt(language, mainText, sourceSrt) {
        if(language === 'vietnamese') {
            ...
            const escapedSrt = escapeForPrompt(sampleSrtContent);
            return `Bạn là một chuyên gia so khớp phụ đề. Dựa vào nội dung truyện đầy đủ và file phụ đề SRT mẫu, hãy chỉnh sửa lại phần nội dung text trong file SRT mẫu sao cho khớp chính xác với nội dung truyện. **TUYỆT ĐỐI GIỮ NGUYÊN MỐC THỜI GIAN**.
**Nội dung truyện đầy đủ (để tham khảo ngữ cảnh):**
---
${escapedMainText}
---
**File SRT mẫu cần chỉnh sửa:**
---
${escapedSrt}
---
**Yêu cầu:** Trả về kết quả dưới dạng JSON object với một key duy nhất là "srt" chứa nội dung SRT đầy đủ đã được chỉnh sửa.`;
        }
        
        let targetLangName = '';
        switch (language) {
            case 'english': targetLangName = 'tiếng Anh'; break;
            case 'japanese': targetLangName = 'tiếng Nhật'; break;
            case 'korean': targetLangName = 'tiếng Hàn'; break;
            default: return null;
        }
...
Ví dụ: { "srt": "1\\n00:00:01,234 --> 00:00:03,456\\nHello everyone." }`;
    ...
    async function handleTranslation() {
        if (apiKeys.length === 0) {
            showToast('Vui lòng nhập và lưu ít nhất một API key.', 'error'); return;
        }

        const targetLang = transLang.value;
        const storyTitle = transTitle.value.trim();
        const storyContent = transSummary.value.trim();
        let genre = transGenreSelect.value === 'custom' ? transGenreCustom.value.trim() : transGenreSelect.value;
        
        if (!genre || !storyTitle || !storyContent) {
            showToast('Vui lòng nhập Thể loại, Tên truyện và Nội dung tóm tắt.', 'error');
            return;
        }

        let channelName = '';
        switch(targetLang) {
            case 'vietnamese': channelName = 'Trần Thiên Minh'; break;
            case 'english': channelName = 'Tran Thien Minh'; break;
            case 'korean': channelName = '쩐 티엔 민 (Tran Thien Minh)'; break;
            case 'japanese': channelName = 'トラン・ティエン・ミン (Tran Thien Minh)'; break;
        }

        const socialMedia = Array.from(document.querySelectorAll('.social-media-item')).map(item => {
        ...
        const prompt = `
            ...
            **Khuôn mẫu kết quả (TUYỆT ĐỐI TUÂN THỦ):**
            ...
            ---------------------------------------------
            🧰 About us:
            ${socialMedia ? '{Danh sách mạng xã hội (giữ nguyên không dịch)}' : ''}
            ---------------------------------------------
        ...
        if (result.success && result.data.formatted_text) {
            ...
            showToast(result.error || 'Dịch thất bại, đã thử tất cả API key.', 'error');
        }
    }
    ...
    function validateInputs() {
        if (!sampleSrtContent) {
            showToast('Vui lòng tải lên một file SRT mẫu chứa thời gian.', 'error'); return false;
        }
        if (!textContentArea.value.trim()) {
            showToast('Vui lòng nhập nội dung truyện chính xác.', 'error'); return false;
        }
        if (apiKeys.length === 0) {
            showToast('Vui lòng nhập và lưu ít nhất một API key.', 'error'); return false;
        }
        return true;
    }

    function resetAllSubtitles() {
        [srtViTextarea, srtEnTextarea, srtJaTextarea, srtKoTextarea].forEach(ta => { ta.value = '';});
        ['en-status', 'ja-status', 'ko-status'].forEach(id => { document.getElementById(id).textContent = ''; });
        document.querySelectorAll('.regenerate-btn').forEach(btn => btn.classList.add('hidden'));
    ...
    function setLoadingState(isLoading, mainMessage = null, detailMessage = null) {
    ...
    function copyToClipboard(targetId) {
        ...
        if (!textarea.value) { showToast('Không có nội dung để sao chép.', 'error'); return; }
        navigator.clipboard.writeText(textarea.value).then(() => showToast('Đã sao chép!', 'success'), () => showToast('Lỗi khi sao chép.', 'error'));
    }

    function downloadSrt(lang) {
        let content = document.getElementById(`srt-${lang}`).value;
        if (!content) { showToast('Không có nội dung để tải xuống.', 'error'); return; }
    ...
    docxFileInput.addEventListener('change', (e) => handleDocxFile(e, textContentArea));
    transDocxFile.addEventListener('change', (e) => handleDocxFile(e, transSummary));
    ...
    resultContainer.addEventListener('click', (e) => {
        ...
        if (button.classList.contains('regenerate-btn') || button.classList.contains('secondary-generate-btn')) {
    ...
    document.querySelectorAll('.copy-btn').forEach(btn => btn.addEventListener('click', (e) => {
        if (e.currentTarget.dataset.target) copyToClipboard(e.currentTarget.dataset.target);
    }));
    document.querySelectorAll('.download-btn').forEach(btn => btn.addEventListener('click', (e) => {
         if (e.currentTarget.dataset.lang) downloadSrt(e.currentTarget.dataset.lang);
    }));
    
    modalCancelBtn.addEventListener('click', hideConfirmModal);
    clearDbBtn.addEventListener('click', () => showConfirmModal('Xác nhận dọn dẹp', 'Bạn có chắc muốn xóa cache?', clearDB));

    // Translator event listeners
    transGenreSelect.addEventListener('change', () => {
        transGenreCustom.classList.toggle('hidden', transGenreSelect.value !== 'custom');
    });
    ...
    copyTranslationBtn.addEventListener('click', () => copyToClipboard('translation-output'));
    ...
    function init() {
        ...
        ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => {
            srtDropZone.addEventListener(eventName, e => { e.preventDefault(); e.stopPropagation(); });
        });
        ['dragenter', 'dragover'].forEach(eventName => srtDropZone.addEventListener(eventName, () => srtDropZone.classList.add('border-teal-500', 'bg-gray-700/50')));
        ['dragleave', 'drop'].forEach(eventName => srtDropZone.addEventListener(eventName, () => srtDropZone.classList.remove('border-teal-500', 'bg-gray-700/50')));
